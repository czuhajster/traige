...
class Factory:
    ...
    def is_person_dead(extractedData):
        ...
        # Check conditions
        if (extractedData['bpm'][0] < critical_thresholds['bpm_low'] or
            extractedData['bpm'][0] > critical_thresholds['bpm_high'] or
            extractedData['spO2'][0] < critical_thresholds['oxygen_saturation_low'] or
            extractedData['systolic'][0] < critical_thresholds['systolic_low'] or
                extractedData['diastolic'][0] < critical_thresholds['diastolic_low']):
    ...
    def data_aggregator(data):
        ...
        return {
            ...
            "status": "Deceased" if is_dead else ("Unhealthy" if data["prediction"] else "Healthy"),
            "data": {
    ...
    def process_data(self, res):
        extractedData = {}
        try:
            bp_data = res['data'][0]['blood_pressure_data']['blood_pressure_samples'][0]
            hr_data = res['data'][0]['heart_data']['heart_rate_data']['detailed']['hr_samples'][0]
